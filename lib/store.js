...
(function (root) {
  ...
  // 统一加 "PF-" 前缀，标识本扩展的内容指纹；为兼容旧备份保留此前前缀。
  ...
  async function importAll(data, mode) {
    ...
    // 防御性校验：必须有合法 PromptFarm 文件标识（防伪造/损坏）
    if (!NS.crypto || !await NS.crypto.checkFileMarker(data._pf_marker)) {
      throw new Error('文件标识无效，可能已损坏或被篡改');
    }
    ...
    // 设置：含 aiConfig 时恢复（apiKey 为加密的 base64 → 用 baseUrl 解密恢复）
    const impSet = data.settings;
    if (impSet && typeof impSet === 'object') {
      const baseSettings = Object.assign({}, impSet);
      const ai = baseSettings.aiConfig;
      delete baseSettings.aiConfig;          // aiConfig 单独存到 ph_ai_config
      if (mode === 'replace') {
        await set(KEYS.settings, Object.assign({}, DEFAULT_SETTINGS, baseSettings));
      } else {
        const cur = await getSettings();
        await set(KEYS.settings, Object.assign({}, cur, baseSettings));
      }
      if (ai && typeof ai === 'object') {
        const next = Object.assign({}, DEFAULT_AI_CONFIG, ai);
        // 解密 apiKey：空/明文(向后兼容旧明文导出) → 原样；加密串 → 用 baseUrl 解密，失败置空
        if (next.apiKey && /^[A-Za-z0-9+/]{40,}={0,2}$/.test(next.apiKey) && NS.crypto) {
          const dec = await NS.crypto.decryptApiKey(next.apiKey, next.baseUrl || '');
          next.apiKey = dec || '';  // 解密失败 → 置空（base 不匹配或损坏）
        }
