...
(function (root) {
  ...
  async function sha256(text) {
    ...
    return null; // 无可用实现时返回 null（不阻塞，仅无指纹）
  }
  ...
  async function replaceAllPrompts(prompts) {
    ...
    for (const p of prompts) {
      ...
      if (!p._hash) await computeHash(p);   // 无指纹则补算
    }
  ...
  async function importAll(data, mode) {
    ...
    if (Array.isArray(data.prompts)) {
      if (mode === 'replace') {
        ...
        // 确保现有数据都有指纹（旧数据可能没有）
        for (const p of cur) { if (!p._hash) await computeHash(p); }
        // 建立现有指纹 → 索引 映射
        const hashIndex = {};
        cur.forEach((p, i) => { if (p._hash) hashIndex[p._hash] = i; });

        for (const incoming of data.prompts) {
          // 为导入项计算指纹（若无或可能过期则重算，保证一致性）
          if (!incoming._hash) await computeHash(incoming);
