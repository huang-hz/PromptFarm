...
(function (root) {
  ...
  const FIELD_WEIGHT = {
    ...
    content: 0.45,
    description: 0.3,
    models: 0.75          // 适配模型（如 "GPT-5.5"、"Claude"）
  };
  ...
  function search(prompts, query, opts) {
    ...
    for (const p of pool) {
      ...
      // 适配模型（含公司名和模型名，如 "OpenAI/GPT-5.5"）
      const modelText = (p.models || []).join(' ');
      const modelScore = scoreField(modelText, q, fieldOpts);
      if (modelScore > 0) { total += modelScore * FIELD_WEIGHT.models; matched.push('models'); }
