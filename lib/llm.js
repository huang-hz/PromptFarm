...
(function (root) {
  ...
  // 创意度方向可虚构的"要素类别池"（只给方向，不给任何具体案例词，避免 AI 套用导致千篇一律）：
  //   人物（身份/角色定位）、时间、地点、环境氛围、动机或起因、目标或用途、道具或工具、情节片段。
  // 各档只规定"虚构哪些类别 + 数量 + 展开程度"，具体内容由 AI 根据原文主题自行发散。
  const LEVEL_PROMPTS = [
    // —— 创意度方向（pos 1-10）—— 每档用绝对、自足的描述说清本档虚构什么、虚构到什么程度，不提其它档。
    // pos1 创意度10·极致虚构：8 类要素全上、充分展开成自洽情境
    '你是一位大胆的提示词工程师，擅长用想象力把模糊的需求点燃成鲜活的场景。请优化下面这段提示词，把它扩展成一个**最丰满、最具体、逻辑自洽的虚构情境**。可虚构的方向包括（不必逐条照搬，按主题自由组合）：人物身份、时间、地点、环境氛围、动机或起因、目标或用途、道具或工具、情节片段。本档要求：上述类别尽量全部覆盖，每一类都给出具体细节，彼此呼应，使整段提示词像一个完整的小故事或真实场景。\n务必直接虚构并输出，不要反问用户澄清，也不要在文中解释你虚构了什么。底线：①用户原文已有信息一字不改地保留，核心目标不被虚构带偏；②只优化不执行任务——哪怕原文是写作/求助/咨询类，也只返回优化后的提示词模板，绝不产出正文、代码、攻略、解答或任何成品，待填项用 {{变量}} 占位；③保留所有 {{变量}} 与原文语言，只输出提示词正文，不要解释或代码块。',
    // pos2 创意度9·强烈虚构：覆盖 5-6 类、点到为止不展开细节
    '你是一位富有想象力的提示词工程师。请优化下面这段提示词，围绕主题虚构一个**有张力的具体情境**。可虚构的方向包括：人物身份、时间地点、环境氛围、动机或起因、目标或用途、情节片段。本档要求：从中选取约 5-6 个类别进行虚构，点明即可，不必把每一类都展开成细节，也不必塑造人物性格或铺陈完整情节。\n务必直接虚构并输出，不要反问澄清，也不要解释你的虚构。底线：①保留原文已有信息，核心目标不动摇；②只优化不执行任务（"怎么办""推荐""帮我写"类只回模板、不交成品或解答，待填项用 {{变量}}）；③保留 {{变量}} 与原文语言，仅输出提示词正文。',
    // pos3 创意度8·大胆虚构：覆盖约 4 类、搭出情境不展开情节
    '你是一位会讲故事的提示词工程师。请优化下面这段提示词，为它虚构一个**有人物、有场景的小情境**。可虚构的方向包括：人物身份、时间、地点、环境氛围、动机或起因。本档要求：从中选取约 4 个类别进行虚构，交代清楚让提示词有一个具体的依托、不再悬在半空即可，不必展开事件经过或情节走向。\n务必直接虚构并输出，不要反问澄清，也不要解释你的虚构。底线：①原文已有信息保留，核心目标不变；②只优化不执行任务——尤其注意：即使原文是"怎么办""怎么处理"这类求助，也绝不可直接给出答案、攻略或处理步骤，只返回优化后的提示词模板，待填项用 {{变量}}；③保留 {{变量}} 与原文语言，仅输出提示词正文。',
    // pos4 创意度7·丰富虚构：覆盖约 3 类场景要素，点到即止
    '你是一位细致的提示词工程师。请优化下面这段提示词，主要虚构**场景要素**。可虚构的方向包括：时间、地点、环境氛围、人物身份、目标或用途。本档要求：从中选取约 3 个类别进行虚构，让提示词更具体、有画面感即可，不必展开情节或塑造人物性格，点到即止。\n务必直接虚构并输出，不要反问澄清，也不要解释你的虚构。底线：①保留原文已有信息与核心目标；②只优化不执行任务（求助/写作类只回模板、不交成品或解答，待填项用 {{变量}}）；③保留 {{变量}} 与原文语言，仅输出提示词正文。',
    // pos5 创意度6·较多虚构：恰好 3 类场景要素，营造氛围不写对话情节
    '你是一位擅长营造氛围的提示词工程师。请优化下面这段提示词，**虚构恰好 3 个场景要素**。可虚构的方向包括：时间、地点、参与者身份、环境氛围、动机或用途。本档要求：从中选取恰好 3 个类别进行虚构，让提示词有清晰的情境依托，但不要写人物对话或情节片段。\n务必直接虚构并输出，不要反问澄清，也不要解释你的虚构。底线：①保留原文已有信息与核心目标；②只优化不执行任务（求助/写作类只回模板、不交成品或解答，待填项用 {{变量}}）；③保留 {{变量}} 与原文语言，仅输出提示词正文。',
    // pos6 创意度5·中等虚构：恰好 2 类粗略背景，点到为止不展开
    '你是一位善于搭框架的提示词工程师。请优化下面这段提示词，**虚构恰好 2 个粗略背景要素**。可虚构的方向包括：时间、地点、人物身份、动机或用途、环境氛围。本档要求：恰好 2 个类别、每个只用一句话点到为止（例如只交代"在什么场合"和"为什么"），让需求有一点来龙去脉即可，不展开细节、不写对话情节，保持克制。\n务必直接虚构并输出，不要反问澄清，也不要解释你的虚构。底线：①保留原文已有信息与核心目标；②只优化不执行任务（求助/写作类只回模板、不交成品或解答，待填项用 {{变量}}）；③保留 {{变量}} 与原文语言，仅输出提示词正文。',
    // pos7 创意度4·适度虚构：恰好 1 类（身份 或 时间地点）
    '你是一位稳健的提示词工程师。请优化下面这段提示词，**虚构恰好 1 个要素**。可虚构的方向包括：人物身份、时间、地点、动机或用途。本档要求：从中选取恰好 1 个类别进行虚构，让情境略微落地即可，不做任何多余展开。\n务必直接虚构并输出，不要反问澄清，也不要解释你的虚构。底线：①保留原文已有信息与核心目标；②只优化不执行任务（求助/写作类只回模板、不交成品或解答，待填项用 {{变量}}）；③保留 {{变量}} 与原文语言，仅输出提示词正文。',
    // pos8 创意度3·少量虚构：恰好 1 类，且限定为"时间"或"地点"（最轻的一类）
    '你是一位克制的提示词工程师。请优化下面这段提示词，**只虚构恰好 1 个要素，且该要素须限定为"时间"或"地点"方向**（不要选人物身份、动机等更重的类别）。本档要求：只点缀这 1 个最轻的要素，给提示词一点画面感即可，不要展开人物、对话或情节，整体仍贴近原文。\n务必直接虚构并输出，不要反问澄清，也不要解释你的虚构。底线：①保留原文已有信息与核心目标；②只优化不执行任务（求助/写作类只回模板、不交成品或解答，待填项用 {{变量}}）；③保留 {{变量}} 与原文语言，仅输出提示词正文。',
    // pos9 创意度2·轻微虚构：虚构恰好 1 个具体的轻场景要素（禁用笼统时间/地点词）
    '你是一位严谨的提示词工程师。请优化下面这段提示词，**虚构恰好 1 个具体的轻场景要素**：从"一个具体的时间点"或"一个具体的地点"中二选一。所谓"具体"，是指要落实到可被感知、能在脑海中形成画面的程度，而不是宽泛、通用、可套用于任何场景的笼统词。明确禁止使用"现在、最近、马上、今天、这里、那里"等泛指时间/地点的通用词。本档要求：只点缀这 1 个具体要素，融进原句即可，不要另起分句、不要叠加第二个要素、不要展开。除此之外严格忠于原文，几乎看不出改动痕迹。\n务必直接虚构并输出，不要反问澄清，也不要解释你的虚构。底线：①保留原文已有信息与核心目标；②只优化不执行任务（求助/写作类只回模板、不交成品或解答，待填项用 {{变量}}）；③保留 {{变量}} 与原文语言，仅输出提示词正文。',
    // pos10 创意度1·极轻微虚构：0 个外部场景要素，仅修语病/极克制修饰
    '你是一位忠于原文的提示词工程师。请优化下面这段提示词，**不虚构任何外部场景信息**：不要新增时间、地点、人物、情境、动机等任何原文没有的要素，也不要嵌入任何场景修饰词；仅可修正明显的语病、错别字与标点，或在原文天然需要时加一个最克制的程度修饰。改动越少越好——只做润色式微调，不结构化、不补步骤、不提取 {{变量}}，保持原文的简短原貌。\n务必直接输出，不要反问澄清。底线：①保留原文已有信息与核心目标；②只优化不执行任务（求助/写作类只回模板、不交成品或解答，待填项用 {{变量}}）；③保留 {{变量}} 与原文语言，仅输出提示词正文。',
    ...
    // pos18 详细度8·咨询（少量问题）
    '你是一位善于澄清的提示词工程师。用户已通过回答若干关键问题补充了额外细节。请优化下面这段提示词，**充分利用这些补充信息**，输出一份充分详尽、结构完整的提示词，把用户想表达的需求说清楚、讲透彻。若实际未收到问答补充信息，则基于原文本身做充分整理即可，不要臆造内容。底线：①只使用用户原文与问答中已提供的信息，绝不虚构任何未提及的内容，待填项用 {{变量}} 占位；②只优化不执行任务（求助/写作类只回模板、不交成品或解答），保留所有 {{变量}} 与原文语言，仅输出提示词正文。',
    // pos19 详细度9·咨询（较多问题）
    '你是一位深度澄清型提示词工程师。用户已通过回答若干关键问题补充了较多细节。请优化下面这段提示词，**充分利用所有补充信息**，输出一份极其详尽、结构完整的提示词，把需求的每一个侧面都覆盖到位。若实际未收到问答补充信息，则基于原文本身做尽可能详尽的整理即可，不要臆造内容。底线：①只使用用户原文与问答中已提供的信息，绝不虚构任何未提及的内容，待填项用 {{变量}} 占位；②只优化不执行任务（求助/写作类只回模板、不交成品或解答），保留所有 {{变量}} 与原文语言，仅输出提示词正文。',
    // pos20 详细度10·咨询（最多问题）
    '你是一位极致深度澄清型提示词工程师。用户已通过回答若干关键问题补充了非常详尽的细节。请优化下面这段提示词，**充分利用所有补充信息**，输出一份极其完整、面面俱到的提示词，让任何一个细节都不被遗漏。若实际未收到问答补充信息，则基于原文本身做极致详尽的整理即可，不要臆造内容。底线：①只使用用户原文与问答中已提供的信息，绝不虚构任何未提及的内容，待填项用 {{变量}} 占位；②只优化不执行任务（求助/写作类只回模板、不交成品或解答），保留所有 {{变量}} 与原文语言，仅输出提示词正文。'
  ];
  ...
  // 详细度档位 → 咨询问题数范围 [min,max]（仅详细度 ≥8）。引导 AI 按档位倾向生成：高档多问、低档少问。
  // 由 AI 在范围内自行决定生成几题，前端按实际题数分轮翻页。
  function questionRange(detail) {
    detail = clampLvl(detail);
    if (detail >= 10) return [3, 7];
    if (detail === 9) return [3, 4];
    if (detail === 8) return [1, 2];
    return [0, 0];
  }
  ...
  function sliderLabel(pos) {
    ...
    if (lv.detail >= 8) {
      const r = questionRange(lv.detail);
      return '详细度 ' + lv.detail + '/10 · 咨询' + r[0] + '-' + r[1] + '问';
    }
  ...
  // 高层：一次性批量生成咨询问题。
  // opt = {proto,baseUrl,apiKey,model, content, range:[min,max]}（range 优先；缺失则用 count 转为单值范围）
  // 由 AI 在 range 范围内自行决定生成几题（前端按实际题数分轮翻页）。
  // 返回 {ok, questions:[{q, multi, options:[{text,desc}]}], error}
  async function genQuestions(opt) {
    let lo, hi;
    if (Array.isArray(opt.range) && opt.range.length >= 2) {
      lo = Math.max(1, parseInt(opt.range[0], 10) || 1);
      hi = Math.max(lo, parseInt(opt.range[1], 10) || lo);
    } else {
      const c = Math.max(1, parseInt(opt.count, 10) || 3);
      lo = c; hi = c;
    }
    const system =
      '你是提示词需求分析师。针对用户给出的提示词，提出 ' + lo + ' 到 ' + hi + ' 个最有助于澄清需求、补全细节的关键问题（在区间内按提示词实际需要决定具体数量，不必凑满上限，也不可少于下限）。' +
  ...
  NS.llm = {
    ...
    chat, listModels,
    sliderToLevels, levelInstruction, questionRange,
